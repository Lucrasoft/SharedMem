...
{
    public class SampleEngine3 : IThreadEngine
    {
        private readonly int playerId;
        private readonly double threshold;
        private readonly Random rnd = new Random(DateTime.Now.Millisecond);

        public SampleEngine3(int playerId, double threshold)
        {
        ...
        public List<ThreadAction> GetActions(GameState state)
        {
            ...
            foreach (var mycell in myCells)
            {
                if (mycell.Value > 1)
                {
                    ...
                    //zijn er cellen om me heen die vrij zijn en zelf daarna ook geen enemy's hebben?
                    //pak die sowieso!
                    ...
                    if (rnd.NextDouble() > (1.0 - (mycell.Value / 255.0)))
                    {
                        ...
                        var transferValue = mycell.Value / 2;
